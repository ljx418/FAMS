...
export class DividendLowVolPortfolioBasketService {
  async build(request: PortfolioBacktestRequest): Promise<PortfolioStrategyDefinition> {
    ...
    try {
      ...
      blockedReasons.push('dividend_low_vol_candidate_snapshot_runtime_health_blocked')
      warnings.push('sqlite_runtime_health_critical:database_disk_image_is_malformed')
      return this.definition(request, [], blockedReasons, warnings, {
        capturedAt: new Date().toISOString(),
        source: 'dividend_low_vol_daily:runtime_health_blocked',
        candidateCount: 0,
    ...
    let rows
    try {
      rows = await prisma.dividendLowVolDaily.findMany({
        where: {
  ...
  private async priceCoverage(symbol: string, startDate: string, endDate: string, minRequiredBars = 2) {
    ...
    try {
      ...
      return {
        count: 0,
        evidenceRef: `runtime_health:sqlite_integrity_check:critical:${symbol}`,
      }
    }
    if (asset) {
      let count
      try {
        count = await prisma.priceHistory.count({
          where: {
            assetId: asset.id,
            isValid: true,
            closePrice: { gt: 0 },
    ...
    let count
    try {
      count = await prisma.marketBarCanonical.count({
        where: {
          symbol,
          market: 'CN',
          timeframe: '1d',
