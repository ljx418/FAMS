...
- 哪些资产触发了基础止盈止损提醒。
- 我如何用 Excel 模板导入、预览、确认和导出本地资产账本。
...
- Excel 导入只维护本地资产账本，不代表交易下单；导入完成后必须保留导入时间、来源和可编辑状态。
- 支持导出当前本地资产账本，导出文件至少包含当前持仓、交易记录和字段说明。
...
- 支持表格导入、手动新增、编辑、删除、导出和标签维护。
- 支持 Excel 模板下载、上传预览、字段校验、错误解释、确认导入和导出当前资产。
- 资产代码可跳转股票或基金详情页。
...
- 默认用户无资产时，Dashboard 必须提供“下载模板 / 导入资产 / 询问 ChatBox”的普通用户入口，不得只展示空数字。
...
- `GET /api/v1/assets/template`：下载资产导入模板。
- `POST /api/v1/assets/parse`：上传 Excel 后预览解析和校验结果。
- `GET /api/v1/assets/export`：导出当前本地资产账本。
...
- 用户能下载 Excel 模板、上传预览、查看校验错误、确认导入，并导出当前本地资产账本。
- Excel 导入和导出只代表本地账本维护，不得被解释为买入、卖出、下单或自动再平衡。
