...
> 2026-09-14 当前口径：`finalFormalReleaseReviewPackageReady=true` 只表示集中人工核查通过后的人类 release 决策包完整，仍必须保持 `productionAdapterEnabled=false`、`formalTradingUnlocked=false`、`autoTradeUnlocked=false`、`canCreateOrder=false`、`orderCreateAllowed=false`。历史 `formalTradingReleaseReviewReady` 不再作为本阶段退出字段，任何自动化流程不得把 review ready 升级为 production ready。
