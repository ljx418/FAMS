...
| --- | --- | --- | --- | --- | --- | --- | --- |
| PX-REQ-001 三入口 | Side Panel / Workspace / FAMS 页面均能进入同一任务 | `background.ts`、`pxExternalBrainBridge.ts`、`WorkspaceApp.tsx` | PX-1/PX-4 | `test:v2-px-entry-matrix` | `entry_matrix_audit.json` | 三入口是否真实可见 | Side Panel + Workspace 已实现；FAMS Host 三页面待 PX4-B |
| PX-REQ-002 三动作 | 按入口查看来源、打开工作台、在工作台定位；Host view_source→Workspace | `intentRoute/3`、`intentRouter.ts`、`OpenInExternalBrainButton.tsx` | PX-1/PX-4 | extension `test:contracts/test:router` | `entry_action_audit.json` | 3×3 目标容器与文案是否清晰 | Side Panel 当前来源→Workspace source_detail 已由 Chrome 重签；Host 动作待 PX4-B |
...
| PX-REQ-017 降级状态 | 未连接/加载/正常/空/失败/恢复/阻断映射十种 lifecycle state；closed 不渲染活动 UI | Workspace/Side Panel state UI、Recovery | PX-2/PX-3/PX-5 | container state + 后续 lifecycle matrix | PX3/PX4A DOM/Chrome；PX5 待生成 | 六种异常/过渡状态是否都有下一步，正常态是否无冲突 | 两容器状态切片诚实且真实 blocked 可见；完整 lifecycle/closed/reload 待 PX5 |
