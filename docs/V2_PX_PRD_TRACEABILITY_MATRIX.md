...
| PX-REQ-009 幂等 | 先写 local ledger；同 key 重放、冲突拒绝、未知结果不自动 dispatch | `idempotencyRegistry.ts`、operation-command/2 | PX-1/PX-4 | extension `test:contracts/test:router` | `idempotency_audit.json`、`dispatch_ledger_reload_audit.json` | 重复/超时是否无隐式副作用 | 目标算法已冻结；当前 v1/runtime 待开发 |
| PX-REQ-010 真实 Chrome | unpacked extension URL/ID/version/build、四视口、trace/hash/network/console | collector + target Chrome evidence/2 | PX-1/PX-6 | extension `verify:real-chrome` | `real_chrome_evidence.json` | URL/ID/四视口/manifest 是否真实 | current v1 已开发；target v2/真实证据未开发 |
...
| PX-REQ-014 单写状态 | background 拥有 route/lifecycle；session/local/TTL/migration 明确 | `background.ts`、Stores、`workspaceStateMigrator.ts` | PX-1/PX-5 | extension `test:lifecycle` | `state_ownership_audit.json` | 三入口是否无状态打架/静默清空 | 状态合同已冻结，代码未开发 |
| PX-REQ-015 最小权限 | optional host 只含 4000；3000 只 external connect；origin allowlist | `wxt.config.ts`、ConnectionGate、Policy | PX-1/PX-2/PX-3 | manifest/network/policy tests | `permission_audit.json` | 安装/连接文案是否透明 | ADR/运行时合同完成，代码未开发 |
