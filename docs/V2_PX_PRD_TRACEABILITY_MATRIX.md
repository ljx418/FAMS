...
implementationMappingStatus=PX1_THROUGH_PRODUCT_PX5_AUTOMATED_ACCEPTED_NEXT_PX6_01_FULL_ACCEPTANCE
...
lifecycleRecoveryEntryStatus=PX5_AUTOMATED_ACCEPTED_PX6_01_ENTRY
...
| PX-REQ-010 真实 Chrome | unpacked extension URL/ID/version/build、恰好四视口、实际像素尺寸/hash、trace/network/console | collector + target Chrome evidence/2 | PX-1/PX-6 | extension `verify:real-chrome`，少项/重复/尺寸不符负例 | `real_chrome_evidence.json` | URL/ID/四视口/manifest 是否真实 | target evidence/2 与分阶段证据已实现；PX6 collector 当前具备重新聚合和逐 hash 复核能力，正式重跑待精确提交 |
| PX-REQ-011 四视口 | 360/420/768/1280 | 两个容器 UI | PX-2/PX-3/PX-6 | Side Panel + Workspace real Chrome | PX4A 360/420 + PX3 768/1280 | 遮挡、溢出、字号是否合格 | 两容器四宽度已验收；PX6 可访问性验证器已用真实数据完成开发态 4/4 重跑，正式证据待精确提交 |
| PX-REQ-012 隐私脱敏 | 证据无 secret/cookie/账户原图 | evidence collector + sanitizer | PX-1/PX-6 | contract + storage/network evidence scan | PX3/PX4A/PX4B/PX5 private evidence | 私有目录是否被 Git 忽略 | 各阶段 runtime 扫描通过；PX6 collector 已实现私有相对路径、HTML 无账户原图/内嵌数据和四锁断言，正式重跑待精确提交 |
...
| PX-REQ-019 可访问性 | 键盘、对比度、点击区、无溢出 | Side Panel/Workspace UI | PX-2/PX-3/PX-6 | `verify:accessibility-chrome` | `accessibility_audit.json` | 键盘走完整核心路径 | 验证器已实现；开发态真实 Chrome 4 视口完成 132 次控件/键盘检查，最低对比度 4.63，失败项 0；正式重跑待精确提交 |
| PX-REQ-020 防规格漂移 | PRD/运行时合同/实体/阶段/证据/commit 一致 | semantic validator + target acceptance manifest/report/2 | PX-0/PX-1/PX-6 | 分批 contract migration + extension `verify:acceptance` | `g1_g7_gate_audit.json` | 按 commit、AC01～10 和目标版本复核 | acceptance `/2` schema/fixtures/semantic validator、collector、12 项篡改负例和人类 HTML 已实现；G1～G7 正式重跑待精确提交 |
...
targetEntitiesRequiringModification=FINAL_ACCEPTANCE_CONTRACTS_COLLECTOR_AND_HUMAN_HTML_REMAIN
targetProductionEntitiesNotDeveloped=NONE_RUNTIME_SCOPE;PX6_ACCEPTANCE_TOOLING_REMAINS
