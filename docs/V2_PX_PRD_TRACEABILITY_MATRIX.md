...
| PX-REQ-015 最小权限 | optional host 只含 4000；3000 只 external connect；每次 API 请求带公开 extension ID header；无 Origin+allowlist ID 才放行，Web Origin+伪造 ID拒绝 | `wxt.config.ts`、ConnectionGate、`FamsApiClient.ts`、Policy | PX-1/PX-2/PX-3 | manifest/network/policy/CORS switch + caller identity matrix | `permission_audit.json`、`cors-switch-audit.json`、PX3 Chrome network | 安装/连接/切换文案是否透明且无 Web Origin/缺 header 旁路 | 方案 A 已由用户批准；待实现与真实 Chrome 重签 |
