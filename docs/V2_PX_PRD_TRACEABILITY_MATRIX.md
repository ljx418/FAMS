...
```text
traceabilityMatrixReady=PARTIAL_DOCUMENTED
...
| --- | --- | --- | --- | --- | --- | --- |
| PX-REQ-001 三入口容器 | sidepanel / workspace / host app 入口图 | sidepanel, Workspace Page, host app bridge | `v2-px-intent-route.schema.json` | `test:v2-px-intent-route` | route scenario result | 三入口是否真实可见 |
...
| PX-REQ-009 幂等 ingest | duplicate ingest journey | PX operation command handler | operation command schema | `test:v2-px-idempotency` | idempotency audit | 重复提交是否去重 |
...
automatedTestsNotImplemented=true
```
