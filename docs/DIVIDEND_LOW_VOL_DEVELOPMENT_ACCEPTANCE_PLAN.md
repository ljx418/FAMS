...
当前网页基础功能已经验收通过，但界面复杂度高。下一阶段新增 UX 专项，不新增交易动作，重点让普通用户快速理解“结论、依据、可信度、下一步”，把高级指标和审计字段折叠到专业模式。
...
7. 普通用户能在红利低波首屏看到当前状态、Top 候选结论、观察区间和下一步按钮，不需要先阅读完整指标表。
8. 专业用户能切换到专业模式，查看完整分数、字段级 evidence、validation matrix 和审计 artifact。
...
- 新增普通模式 / 专业模式展示规则。
- 红利低波候选默认改为结论卡：可研究、仅观察、数据不足、风险剔除。
- 候选详情保留完整指标和审计证据，但默认折叠。
- 策略回测页面顶部增加“收益最高、回撤最低、数据最可信、当前阻断”摘要。
- 任务中心和审计报告增加普通用户摘要。
- 统一状态色和普通话解释，减少一屏内标签堆叠。

验收标准：

- 普通用户能在 30 秒内判断当前页面能做什么、系统结论是什么、哪里不可信、下一步做什么。
- 红利低波页面默认不以完整分数表作为首屏主内容。
- 组合回测结果默认不只展示曲线，还必须解释策略对比结论和数据缺口。
- 所有正式交易锁定、数据不足、模型验证不足仍必须可见，不得被体验优化隐藏。
- 桌面、平板、移动端截图不得出现明显文字溢出、按钮含义不清或标签堆叠不可读。
