...
const DashboardStatCard: React.FC<{
...
}> = ({ icon, title, value, detail, tone = 'blue' }) => {
  const toneClass = {
    blue: 'text-blue-600',
  ...
  return (
    <Card className="fams-stat-card fams-pressable">
      <div className={`fams-stat-icon ${toneClass}`}>{icon}</div>
      <div className="fams-muted text-sm">{title}</div>
      <div className="mt-2 break-words text-2xl font-semibold leading-tight text-slate-950">{value}</div>
      {detail && <div className="fams-muted mt-2 text-sm">{detail}</div>}
    </Card>
  )
}

const UserTaskWorkbench: React.FC = () => (
  <Card className="fams-card" styles={{ body: { padding: 18 } }}>
    ...
    <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
      {workbenchTasks.map((task) => (
        <button
          ...
          className="fams-pressable rounded-lg border border-slate-200 bg-slate-50 p-4 text-left"
        >
  ...
  const columns: ColumnsType<PositionRecord> = [
    {
      ...
      render: (_, record) => (
        <div>
          <div className="break-all font-medium text-slate-950">{record.asset.symbol}</div>
          <div className="break-all text-xs text-slate-500">{record.asset.name}</div>
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      align: 'right',
      render: (value: number) => <span className="text-slate-950">{formatWan(value)}</span>,
    },
    {
      title: '实际仓位',
  ...
  return (
    <div className="fams-page space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <div className="fams-eyebrow">普通用户首页</div>
          <h1 className="fams-page-title mb-0">总览</h1>
          <p className="fams-muted mb-0 mt-2">从 ChatBox 和工作台开始，必要时再进入专家模块核对证据。</p>
        </div>
      ...
      <Row gutter={[16, 16]}>
        <Col xs={24} sm={12} lg={6}>
          <DashboardStatCard
            icon={<WalletOutlined />}
            title="总市值"
            value={formatStatisticWan(summary.totalValue / 10000)}
            detail="来自当前本地持仓"
            tone="blue"
          />
        </Col>
        <Col xs={24} sm={12} lg={6}>
          <DashboardStatCard
            icon={<BankOutlined />}
            title="总成本"
            value={formatStatisticWan(summary.totalCost / 10000)}
            detail="用于估算浮动盈亏"
            tone="amber"
          />
        </Col>
        <Col xs={24} sm={12} lg={6}>
          <DashboardStatCard
            icon={<RiseOutlined />}
            title="浮动盈亏"
            value={formatStatisticWan(summary.totalPnl / 10000)}
            detail={(
              <>
                收益率 <span className={summary.totalPnlPercent >= 0 ? 'text-emerald-600' : 'text-rose-600'}>
                  {formatPercent(summary.totalPnlPercent)}
                </span>
              </>
            )}
            tone={summary.totalPnl >= 0 ? 'emerald' : 'rose'}
          />
        </Col>
        <Col xs={24} sm={12} lg={6}>
          <Card className="fams-stat-card fams-pressable">
            <div className="fams-stat-icon text-rose-600"><AlertOutlined /></div>
            <GaugeChart
              value={riskScore}
              max={100}
              label="风险评分"
            ...
            <div className="mt-[-2px] text-center">
              <div className="text-2xl font-bold text-amber-600 leading-none">{riskScore}</div>
      ...
      <Row gutter={[16, 16]}>
        <Col xs={24} lg={8}>
          <Card title={<span className="text-slate-950">现金仓位</span>} className="fams-card fams-pressable">
            <Statistic
              title={<span className="fams-muted">现金市值</span>}
              value={summary.cashValue / 10000}
              formatter={formatStatisticWan}
              valueStyle={{ color: '#2563eb', fontSize: 22, lineHeight: 1.2, wordBreak: 'break-all' }}
            />
            <div className="mt-4">
              <div className="fams-muted mb-2 flex justify-between text-sm">
                <span>现金占比</span>
        ...
        <Col xs={24} lg={8}>
          <Card title={<span className="text-slate-950">持仓数量</span>} className="fams-card fams-pressable">
            <Statistic
              title={<span className="fams-muted">开放仓位</span>}
              value={summary.positionsCount}
              suffix="个"
              valueStyle={{ color: '#2563eb' }}
            />
            <div className="fams-muted mt-4 text-sm">
              统计来自后端 `PositionService`，Dashboard 与资产/仓位页使用同一套口径。
            </div>
          </Card>
        </Col>
        <Col xs={24} lg={8}>
          <Card title={<span className="text-slate-950">告警状态</span>} className="fams-card fams-pressable">
            <Statistic
              title={<span className="fams-muted">活跃告警</span>}
      ...
      <Row gutter={[16, 16]}>
        <Col xs={24} lg={12}>
          <Card title={<span className="text-slate-950"><PieChartOutlined /> 资产类型配置</span>} className="fams-card card-lg">
            {allocationData.length > 0 ? (
              ...
              <div className="fams-empty-state">
                <InboxOutlined className="text-3xl text-blue-600" />
                <div>
                  <div className="font-semibold text-slate-950">暂无资产配置数据</div>
                  <div className="fams-muted mt-1 text-sm">去资产管理页下载模板并导入 Excel 后，这里会显示配置图。</div>
                </div>
                <Button type="primary" href="/assets">去导入资产</Button>
              </div>
            )}
          </Card>
        </Col>
        <Col xs={24} lg={12}>
          <Card title={<span className="text-slate-950"><DollarOutlined /> 主要持仓</span>} className="fams-card card-lg">
            <Table
              columns={columns}
        ...
        <Col xs={24}>
          <Card className="fams-card">
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <div>
                <div className="text-base font-medium text-slate-950">AI标的研究与选股</div>
                <div className="fams-muted mt-1 text-sm">
